...
public class GoodsFrontController extends HttpServlet 
	...
	protected void doProcess(HttpServletRequest request, 
		...
		if(command.equals("/goodsList.goods")) {
			...
			// 번호 자동 부여
			GoodsAutoNumController action =
					new GoodsAutoNumController();
			action.execute(request);
			RequestDispatcher dispatcher =
			...
			GoodsDetailController goodsDetailController =
					new GoodsDetailController();
			goodsDetailController.execute(request);
			RequestDispatcher dispatcher = 
			...
			GoodsUpdateController goodsUpdateController =
